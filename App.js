...
/* Todo:
 *  1. Change buttons to pull down tree menu
 *  2. Maybe cache next page?
 *
 * */
export default class simpleSpanish extends Component {
    ...
    render() {
        ...
        return ( <
            ...
            WebView source = { this.pages[this.state.index].file }
            originWhitelist = {
                ['*']
            }
            style = { styles.webview }
            injectedJavaScript = { `const meta = document.createElement('meta'); meta.setAttribute('content', 'width=width, initial-scale=0.7, maximum-scale=2, user-scalable=2.0'); meta.setAttribute('name', 'viewport'); document.getElementsByTagName('head')[0].appendChild(meta); ` }
