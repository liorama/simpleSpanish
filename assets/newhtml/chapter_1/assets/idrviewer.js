/*! IDRViewer - v1.8.2 | Copyright 2018 IDRsolutions */
!function () {
	"use strict";
	var a, b, c, d, e, f, g, h, i, j, k, l, m = {
		...
		ZOOM_AUTO: "auto"
	}, n = 1, o = 1, p = 0, q = 0, r = m.ZOOM_AUTO, s = !0, t = [], u = [], v = 10, w = {}, x = !1, y = "", z = [], A = !1, B = "continuous_horizontal";
	m.setup = function (n) {
		n || (n = IDRViewer.config), x = !0, d = /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent), f = "svgz" === n.pageType, e = f || "svg" === n.pageType, g = n.bounds, p = n.pagecount, n.url && (y = n.url), E.setup(), b = W("idrviewer");
		...
		for (var w = 0; w < p; w++) if (g[w][0] != g[0][0] || g[w][1] != g[0][1]) {
			s = !1;
			break
		}
		switch (k) {
		...
		c.setup(o), X.addClass(b, "layout-" + c.toString()), Q(l), c.goToPage(o), E.setPage(o, !0);
		var F = {selectMode: N.currentSelectMode, isMobile: d, layout: c.toString(), availableLayouts: z, isFirstPage: 1 === o, isLastPage: c.isLastPage(o)};
	...
	var C = function (a, b) {
		...
		return c.isVisible = function () {
			...
			E.getState(b) !== D.LOADED && E.getState(b) !== D.HIDDEN || (E.setState(b, D.UNLOADED), a.removeChild(a.firstChild))
		}, c.load = function () {
			var c = E.getState(b);
			if (c === D.HIDDEN && (E.setState(b, D.LOADED), a.firstChild.style.display = "block"), c === D.UNLOADED) if (E.setState(b, D.LOADING), e) {
				var d = function () {
				...
				var i = function () {
					...
					c.setAttribute("class", "page-inner"), c.setAttribute("src", y + b + ".html"), c.setAttribute("style", "width: " + g[b - 1][0] + "px; height: " + g[b - 1][1] + "px; position: relative; border: 0;"), c.onload = function () {
						E.setState(b, D.LOADED), m.fire("pageload", {page: b})
					}, a.appendChild(c)
				}, k = function () {
					var c = new XMLHttpRequest;
					c.open("GET", y + b + ".html", !0), c.onload = function () {
						if (c.status >= 200 && c.status < 400) {
							...
							var f = function () {
								E.setState(b, D.LOADED), this && this.removeEventListener("load", f), m.fire("pageload", {page: b})
							...
							o && (o.parentNode.removeChild(o), A || (document.head.appendChild(o), A = !0)), X.addClass(e, "page-inner");
							for (var p = function () {
	...
	}, D = {LOADING: "loading", HIDDEN: "hidden", UNLOADED: "unloaded", LOADED: "loaded"}, E = function () {
		...
		var m = function (a, b) {
			...
			if (t[a].load(), i < h) for (var d = 1; d < g / 2 && (o(a - d) && (t[a - d].isVisible() || t[a - d].load()), i !== h) && (o(a + d) && (t[a + d].isVisible() || t[a + d].load()), i !== h); d++) ;
		...
		o != a && (o = a, E.setPage(a), m.fire("pagechange", {page: o, pagecount: p, isFirstPage: 1 === o, isLastPage: c.isLastPage(a)}))
	}, H = function () {
		var c = {};
		c.setup = function () {
		}, c.unload = function () {
			for (var b = 1; b <= p; b++) u[b].style.marginLeft = "", u[b].style.marginTop = "", X.removeClass(u[b], "current", "prev", "next", "before", "after");
		...
		var d = function (a) {
			for (var b = 1; b <= p; b++) X.removeClass(u[b], "current", "prev", "next", "before", "after"), b < a ? X.addClass(u[b], "before") : b > a && X.addClass(u[b], "after");
			X.addClass(u[a], "current"), a - 1 >= 1 && X.addClass(u[a - 1], "prev"), a + 1 <= p && X.addClass(u[a + 1], "next")
		};
		return c.updateLayout = function () {
			var c = Math.floor(g[o - 1][0] * n), d = 0, e = b.clientWidth - v;
			e > c ? d = (e - c) / 2 : e = c;
			var f = Math.floor(g[o - 1][1] * n), h = 0, i = b.clientHeight - v;
			...
			return {width: g[o - 1][0], height: g[o - 1][1]}
		...
		var e = function (a) {
			for (var b = 1; b <= p; b++) X.removeClass(u[b], "current", "prev", "next", "before", "after");
			if (X.addClass(u[a], "current"), c(a) && X.addClass(u[a + 1], "current"), 1 == a && (a = 0), a + 2 <= p && (X.addClass(u[a + 2], "next"), a + 3 <= p && X.addClass(u[a + 3], "next")), a - 1 > 0 && (X.addClass(u[a - 1], "prev"), a - 2 > 0 && X.addClass(u[a - 2], "prev")), a + 4 <= p) for (b = a + 4; b <= p; b++) X.addClass(u[b], "after");
			if (a - 3 > 0) for (b = a - 3; b > 0; b--) X.addClass(u[b], "before")
		};
		return d.updateLayout = function () {
			var d = c(o), e = Math.floor(g[o - 1][0] * n), f = d ? Math.floor(g[o][0] * n) : 0, h = 2 * Math.max(e, f), i = Math.max(h, b.clientWidth - v), j = i / 2 - e, k = j + Math.floor(g[o - 1][0] * n), l = Math.floor(g[o - 1][1] * n),
				m = d ? Math.floor(g[o][1] * n) : l, q = Math.max(l, m, b.clientHeight - v), r = (q - l) / 2, s = (q - m) / 2;
			...
			var a = c(o), b = Math.floor(g[o - 1][0]), d = a ? Math.floor(g[o][0]) : 0, e = Math.floor(g[o - 1][1]), f = a ? Math.floor(g[o][1]) : 0;
		...
		var f = function () {
			...
			var a, b;
			if (u[1].getBoundingClientRect().top > 0) G(1); else for (a = 1; a <= p; a++) {
		...
		var c = {}, d = 0, e = 0, f = [], h = 0, i = 0, j = 0, k = 0;
		return c.setup = function () {
			...
			if (u[a].style.transition = "0.5s", u[a].style.transform = "translate3D(0, 0, 0)", a === o) a > 1 && (u[a - 1].style.transition = "0.5s", u[a - 1].style.transform = "translate3D(" + -j + "px, 0, 0)"), a < p && (u[a + 1].style.transition = "0.5s", u[a + 1].style.transform = "translate3D(" + j + "px, 0, 0)"); else if ("right" === b) {
			...
			var c = e * n, f = d * n, g = (j - f) / 2, h = (k - c) / 2;
		...
		f.setup = function () {
			...
			this.currentSelectMode = m.SELECT_SELECT, X.removeClass(h, "panning"), h.removeEventListener("mousedown", i), document.removeEventListener("mouseup", j), h.removeEventListener("mousemove", k)
		};
		var i = function (b) {
			return b = b || window.event, X.addClass(h, "mousedown"), a = b.clientX, c = b.clientY, g = !0, !1
		}, j = function () {
			X.removeClass(h, "mousedown"), g = !1
		}, k = function (d) {
		...
		return f.enablePanning = function () {
			this.currentSelectMode = m.SELECT_PAN, e ? M(window) : L(window), X.addClass(h, "panning"), h.addEventListener("mousedown", i), document.addEventListener("mouseup", j), h.addEventListener("mousemove", k)
	...
	m.setSelectMode = function (a) {
		if (x) {
			if (d) return;
			a == m.SELECT_SELECT ? N.enableTextSelection() : N.enablePanning(), m.fire("selectchange", {type: a})
	...
	var O, P = function (a, b, c, d, e) {
		...
		b.scrollTop = b.scrollHeight * f, q++, q % 2 === 1 && Q(), m.fire("zoomchange", {zoomType: r, zoomValue: n, isMinZoom: d, isMaxZoom: e})
	};
	m.zoomIn = function () {
		Q(T())
	}, m.zoomOut = function () {
	...
	var R = [.25, .5, .75, 1, 1.25, 1.5, 2, 2.5, 3, 3.5, 4], S = [m.ZOOM_AUTO, m.ZOOM_FITPAGE, m.ZOOM_FITHEIGHT, m.ZOOM_FITWIDTH, m.ZOOM_ACTUALSIZE], T = function () {
		for (var a = n, b = R[R.length - 1], c = 0; c < R.length; c++) if (R[c] > a) {
			b = R[c];
			break
		}
		for (c = 0; c < S.length; c++) {
			var d = V(S[c]);
			if (d > a && d <= b) return S[c]
		}
		return b
	}, U = function () {
		for (var a = n, b = R[0], c = R.length - 1; c >= 0; c--) if (R[c] < a) {
			b = R[c];
			break
		}
		for (c = 0; c < S.length; c++) {
			var d = V(S[c]);
			if (d >= b && d < a) return S[c]
		}
		return b
	}, V = function (a) {
		var d = c.getZoomBounds(), e = (b.clientWidth - v) / d.width, f = (b.clientHeight - v) / d.height, g = parseFloat(a);
	...
	m.goToPage = function (a, b) {
		...
		x ? (c.unload(), X.removeClass(b, "layout-" + c.toString()), a == m.LAYOUT_PRESENTATION ? c = H : a == m.LAYOUT_MAGAZINE ? c = I : a == m.LAYOUT_CONTINUOUS ? c = J : a == B && (c = K), c.setup(o), X.addClass(b, "layout-" + c.toString()), Q(IDRViewer.ZOOM_AUTO), c.goToPage(o), m.fire("layoutchange", {layout: a})) : k = a
	...
	if (window) {
		...
		window.Draggable = function (a) {
			...
			var e = this;
			if (this.element.onmousemove = null, this.html.addEventListener) if (this.isTouch) {
				this.element.addEventListener("touchstart", b), this.html.addEventListener("touchmove", d), this.html.addEventListener("touchend", c);
				try {
					b(event)
				} catch (f) {
				}
			} else this.element.addEventListener("mousedown", b), this.html.addEventListener("mousemove", d), this.html.addEventListener("mouseup", c); else this.html.attachEvent && (this.html.attachEvent("onmousedown", b), this.html.attachEvent("onmousemove", d), this.html.attachEvent("onmouseup", c));
			a.onclick = function () {
